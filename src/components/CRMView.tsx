...
import { Search, ChevronLeft, ChevronRight, Briefcase, UserCheck, Users, Trophy } from 'lucide-react';
  ...
  const fetchCRMData = async () => {
    ...
    try {
      if (activeTab === 'contacts') {
        const res = await api.getContacts({ page, pageSize: 10, search: searchTerm });
        setData(res.data);
        setTotal(res.total);
      } else {
        const res = await api.getDeals({ page, pageSize: 10, search: searchTerm, type: activeTab });
        setData(res.data);
        setTotal(res.total);
      }
    } catch (error) {
            ...
            <thead className="text-xs text-gray-500 uppercase bg-gray-50 border-b border-gray-100">
              <tr>
                {activeTab === 'contacts' ? (
                  ...
                  <>
                    <th className="px-6 py-3 font-medium">{activeTab === 'won' ? 'Data Venda' : 'Data Criação'}</th>
                  ...
                  if (activeTab === 'contacts') {
                    ...
                    return (
                      <tr key={contact.id} className="hover:bg-gray-50/50">
                        ...
                        <td className="px-6 py-4 text-gray-500">{contact.last_conversion_date ? formatDate(contact.last_conversion_date) : '-'}</td>
                      </tr>
                    );
                  } else {
                    ...
                    return (
                      <tr key={deal.id} className="hover:bg-gray-50/50">
                        <td className="px-6 py-4">{formatDate(activeTab === 'won' ? deal.win_at! : deal.created_at)}</td>
                        <td className="px-6 py-4 font-medium text-gray-900">{deal.organization_name}</td>
                        <td className="px-6 py-4 text-gray-500">{deal.user_name}</td>
                        <td className="px-6 py-4">
                          <div className="flex flex-col">
                            <span className="text-gray-900">{deal.email}</span>
                            <span className="text-xs text-gray-400">{deal.phone}</span>
                          </div>
                        </td>
                        <td className="px-6 py-4">
                        ...
                        <td className="px-6 py-4 text-right font-medium text-gray-900">{formatCurrency(deal.amount_total)}</td>
                      </tr>
                    );
                  }
                })
